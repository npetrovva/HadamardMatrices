...
void WriteToFile(deque<uint64_t> dqFin2, int counter, string file_write) {
    ofstream out;
    if (file_write != "\0") {
        out.open(file_write, ios::app);
    } else {
        out.open("Rres28-1.txt", ios::app);
    }
...
void SwitchHS(uint64_t *arr) { //операция переключения с помощью отрицания F1 и G1
    ...
    //cout << "neg = " << neg << endl;
    uint64_t mask = (((uint64_t)1 << 4) - 1) << (N - 4); // 1111 -> 11110000000000000000
    cout << "mask = " << mask << endl;
                            ...
                            if (FindG2(dqFin)) {
                                if (FindG3(dqFin)) {
                                    if (FindG4(dqFin)) {
                                        ...
                                        WriteToFile(dqFin2, counter, file_write);
                                        counter += 8;
                                    } else {
                                        counter += 8;
                                        continue;
