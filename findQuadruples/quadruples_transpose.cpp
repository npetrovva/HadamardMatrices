...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    for (auto firstQuadr: MatrixWithQuadr) { //матрица в сете
        ...
        uint64_t tmp_matrix[N];
        memset(tmp_matrix, 0, N * sizeof(uint64_t));
        int k = 0;
        for (auto v: firstQuadr) {
            tmp_matrix[k++] = v;
        }
        uint64_t matrix_transp[N];
        memset(matrix_transp, 0, N * sizeof(uint64_t));
        ColumnToString(tmp_matrix, matrix_transp);
        deque<uint64_t> qmatrx;
        for (int j = 0; j < N; j++) {
            qmatrx.push_back(matrix_transp[j]);
        }
